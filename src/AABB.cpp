...
AABB::AABB(glm::vec4 min, glm::vec4 max)
...
AABB::AABB(Mesh *mesh) {
  ...
  for (std::vector<Vertex>::iterator it = mesh->vertexData.begin();
       it != mesh->vertexData.end(); ++it) {
  ...
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "Mesh bounds are: X(%f->%f) Y(%f->%f) Z(%f->%f) ", min.x,
