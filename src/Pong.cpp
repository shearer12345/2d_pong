...
Pong::Pong(std::string windowName) : Game::Game(windowName) {
  glslProgram = new GLSLProgram();

  Texture *textureOpenGL = new Texture("assets/Opengl-logo.svg.png");
  ...
  trackingCamera =
      new TrackingOrthoCamera(glslProgram, ball, glm::vec2(220.0f));
  orthoCamera = new OrthoCamera(glslProgram, glm::vec3(0.0f, 0.0f, 1.0f),
                                glm::vec2(220.0f));
  camera = orthoCamera;
}
...
void Pong::render() {
  ...
  for (std::vector<Model *>::iterator it = modelList.begin();
       it != modelList.end(); ++it) {
    Model *model = *it;
    model->draw();
  }
  ...
  for (std::vector<Model *>::iterator it = scoreMarkerList.begin();
       it != scoreMarkerList.end(); ++it) {
    Model *model = *it;
    model->draw();
  }
}
...
void Pong::incrementScore(int player) {
  ...
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "  incrementScore for player %d", player);

  GLfloat direction = (player == 0) ? -1.0f : 1.0f;

  auto *textureScore = new Texture("assets/Opengl-logo.svg.png");

  Model *scoreMarker =
      new Model("scoreMarker", triangle, textureScore,
                glm::vec3(direction * 8 * (9 - score[player]), -80.0f, 0.0f),
                glm::vec3(0.0f, 20.0f, 0.0f), false);
...
void Pong::handleBallCollision(Model *other) {
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "  Ball Collision with %s", other->name.c_str());
  ...
  if (other == wallLeft) {
    ...
    incrementScore(1);
    SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                   "  Hit wallLeft. Score now %d:%d", score[0], score[1]);
  } else if (other == wallRight) {
    ball->position = glm::vec3(0.0f);
    incrementScore(0);
    SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                   "  Hit wallRight. Score now %d:%d", score[0], score[1]);
  }

  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "  Ball Collision complete");
}

void Pong::handleBatCollision(Model *bat, Model *other) {
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "  Bat Collision with %s", other->name.c_str());
  if (other != ball) {
  ...
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_DEBUG,
                 "  Bat Collision complete");
}
void Pong::updateSimulation(double simLength) // update simulation with
                                                       // an amount of time to
                                                       // simulate for (in
                                                       // seconds)
{
  ...
  // collision detection
  for (std::vector<Model *>::iterator it2 = modelList.begin();
       it2 != modelList.end(); ++it2) {
    Model *model2 = *it2;
    for (std::vector<Model *>::iterator it1 = modelList.begin();
         it1 != modelList.end(); ++it1) {
      Model *model1 = *it1;
      if (it1 == it2)
        break;
      ...
      SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_VERBOSE,
                     "Testing for collision between %s and %s",
                     model1->name.c_str(), model2->name.c_str());
      bool intersection = Model::intersects(model1, model2);
      if (intersection) {
        if (model1 == ball)
          handleBallCollision(model2);
        else if (model2 == ball)
          handleBallCollision(model1);

        if (model1 == batLeft)
          handleBatCollision(batLeft, model2);
        else if (model2 == batLeft)
          handleBatCollision(batLeft, model1);

        if (model1 == batRight)
          handleBatCollision(batRight, model2);
        else if (model2 == batRight)
          handleBatCollision(batRight, model1);
      }
    }
  }
  SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_VERBOSE,
  ...
  for (std::vector<Model *>::iterator it = modelList.begin();
    ...
    SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_VERBOSE,
                   "  %s->velocity: %f, %f, %f", model->name.c_str(), vel.x,
                   vel.y, vel.z);
    SDL_LogMessage(SDL_LOG_CATEGORY_RENDER, SDL_LOG_PRIORITY_VERBOSE,
                   "  %s->position: %f, %f, %f", model->name.c_str(),
